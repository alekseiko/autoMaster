...
class AutoMaster: 
		...
		for (issue_key, issue_assignee) in issues:
			if self.__accept_issue_phase(issue_key, issue_assignee):
				if not config.is_bulk:
					...
					try:
						self.__git.push(config.repo_urls[MAIN_GIT], \
						...
						self.__note.notify(self.main_commiter, "Push to server is Fail")
	...
	def __accept_issue_phase(self, issue_key, issue_assignee)
		...
		LOGGER.debug("accept phase task:%s" %s)
		holder_name = config.jira_to_holders[issue_assignee]
